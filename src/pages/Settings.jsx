...
import {
    ...
    AlertTriangle, X, Check, Shield, Loader, MapPin
} from 'lucide-react'
import { useProfile, useCategories, useSites } from '../hooks/useSettings'
import CategoryModal from '../components/settings/CategoryModal'
import SiteModal from '../components/settings/SiteModal'
...
function ProfileSection() {
    ...
    if (profile && !initDone) { setName(profile.full_name || ''); setInitDone(true) }
    ...
    async function handleSaveName(e) {
        e.preventDefault()
        try { await updateProfile({ full_name: name }); showToast('Profile updated.') }
    ...
    async function handleAvatarChange(e) {
        ...
        if (!file) return
        try { await uploadAvatar(file); showToast('Avatar updated.') }
        ...
        <div className="st-card">
            ...
            {loading ? (
                <div className="st-skeleton-rows">
                    {[100, 200, 60].map((w, i) => <div key={i} className="st-skeleton" style={{ width: w, height: 16 }} />)}
                </div>
            ) : (
                <div className="st-profile-body">
                    <div className="st-avatar-wrap">
                        <div className="st-avatar">
                            ...
                            <button className="st-avatar-edit" onClick={() => fileRef.current?.click()} disabled={saving} title="Change avatar">
                        ...
                        <div>
                            ...
                            <div className="st-avatar-role" style={{ color: role.color }}>
                                <Shield size={12} strokeWidth={1.5} /> {role.label}
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="st-role-info">
                        <div className="st-role-badge" style={{ background: role.color + '18', color: role.color, borderColor: role.color + '30' }}>
                            <Shield size={13} strokeWidth={1.5} /> {role.label}
                        </div>
...
function CategoriesSection() {
    ...
    async function handleSave(payload) {
        if (editing) { await updateCategory(editing.id, payload); showToast('Category updated.') }
        else { await createCategory(payload); showToast('Category added.') }
    }

    async function handleDelete() {
        if (!deleteTarget) return
        setDeleteLoading(true)
        try { await deleteCategory(deleteTarget.id); showToast('Category deleted.', 'danger') }
    ...
    return (
        <div className="st-card">
            <div className="st-card-header">
                <SectionHeader icon={Tag} title="Categories" sub="Manage CAPEX and OPEX categories used across resources and projects" />
                <button className="btn-primary" onClick={openAdd}><Plus size={14} strokeWidth={2} /> Add category</button>
            </div>

            {error && <div className="rc-error" style={{ marginBottom: 16 }}><AlertTriangle size={14} /> {error}</div>}
            ...
            {loading ? (
                ...
                <div className="rc-empty" style={{ padding: '40px 0' }}>
                    ...
                    <button className="btn-primary" style={{ marginTop: 16 }} onClick={openAdd}><Plus size={14} /> Add category</button>
                </div>
            ) : (
                        ...
                        <div key={label} className="st-cat-group">
                            ...
                            <div className="st-cat-list">
                                {items.map((cat, i) => {
                                    ...
                                    return (
                                        <div key={cat.id} className="st-cat-row" style={{ animationDelay: `${i * 30}ms` }}>
                                            <div className="st-cat-icon"><Tag size={14} strokeWidth={1.5} /></div>
                                            <div className="st-cat-info">
                                                <div className="st-cat-name">{cat.name}</div>
                                                {cat.description && <div className="st-cat-desc">{cat.description}</div>}
                                            </div>
                                            <div className="st-cat-resource-count" title={`${count} resource${count !== 1 ? 's' : ''} using this category`}>
                                                {count > 0 && (
                                                    <><span className="st-cat-count-num">{count}</span><span className="st-cat-count-label">resource{count !== 1 ? 's' : ''}</span></>
                                                )}
                                            </div>
                                            <div className="st-cat-actions">
                                                <button className="rc-btn" onClick={() => openEdit(cat)} title="Edit"><Pencil size={13} strokeWidth={1.5} /></button>
                                                <button className="rc-btn rc-btn-danger" onClick={() => setDeleteTarget({ ...cat, resourceCount: count })} title="Delete"><Trash2 size={13} strokeWidth={1.5} /></button>
                                            </div>
                                        </div>
                                    )
                                })}
            ...
            <CategoryModal open={modalOpen} onClose={() => setModalOpen(false)} onSave={handleSave} category={editing} />
                    ...
                    <div className="modal-box" style={{ maxWidth: 420 }}>
                        ...
                        <div style={{ padding: '12px 0 20px' }}>
                            ...
                            {deleteTarget.resourceCount > 0 ? (
                                ...
                                <p style={{ fontSize: 13, color: '#9a9790' }}>This category has no resources linked to it. Safe to delete.</p>
                            )}
                        </div>
...
function SitesSection() {
    const { sites, projectCounts, loading, error, createSite, updateSite, deleteSite } = useSites()
    const [modalOpen, setModalOpen] = useState(false)
    const [editing, setEditing] = useState(null)
    const [deleteTarget, setDeleteTarget] = useState(null)
    const [deleteLoading, setDeleteLoading] = useState(false)
    ...
    async function handleSave(payload) {
        if (editing) { await updateSite(editing.id, payload); showToast('Site updated.') }
        else { await createSite(payload); showToast('Site added.') }
    }

    async function handleDelete() {
        if (!deleteTarget) return
        setDeleteLoading(true)
        try { await deleteSite(deleteTarget.id); showToast('Site deleted.', 'danger') }
        catch (e) { showToast(e.message, 'danger') }
        setDeleteLoading(false)
        setDeleteTarget(null)
    }
        ...
        <div className="st-card">
            <div className="st-card-header">
                <SectionHeader
                    icon={MapPin}
                    title="Sites"
                    sub="Network sites and locations assigned to projects"
                />
                <button className="btn-primary" onClick={openAdd}><Plus size={14} strokeWidth={2} /> Add site</button>
            </div>
            ...
            {loading ? (
                <div className="st-skeleton-rows">
                    {[1, 2, 3].map(i => <div key={i} className="st-skeleton" style={{ height: 48 }} />)}
                </div>
            ) : sites.length === 0 ? (
                <div className="rc-empty" style={{ padding: '40px 0' }}>
                    <div className="rc-empty-icon">📍</div>
                    <p>No sites yet</p>
                    <span>Add sites to assign them to projects.</span>
                    <button className="btn-primary" style={{ marginTop: 16 }} onClick={openAdd}><Plus size={14} /> Add site</button>
                </div>
            ) : (
                <div className="st-cat-list" style={{ marginTop: 8 }}>
                    {sites.map((site, i) => {
                        const count = projectCounts[site.id] || 0
                        return (
                            <div key={site.id} className="st-cat-row" style={{ animationDelay: `${i * 30}ms` }}>
                                <div className="st-cat-icon">
                                    <MapPin size={14} strokeWidth={1.5} style={{ color: '#c9a84c' }} />
                                </div>
                                <div className="st-cat-info">
                                    <div className="st-cat-name" style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                                        {site.name}
                                        {site.code && (
                                            <span style={{
                                                fontSize: 10, fontWeight: 700, letterSpacing: 0.5,
                                                background: '#f0ede8', color: '#7a7872',
                                    ...
                                    {site.address && <div className="st-cat-desc">{site.address}</div>}
                                </div>
                                <div className="st-cat-resource-count" title={`${count} project${count !== 1 ? 's' : ''} using this site`}>
                                    {count > 0 && (
                                        <><span className="st-cat-count-num">{count}</span><span className="st-cat-count-label">project{count !== 1 ? 's' : ''}</span></>
                                    )}
                                </div>
                                <div className="st-cat-actions">
                                    <button className="rc-btn" onClick={() => openEdit(site)} title="Edit"><Pencil size={13} strokeWidth={1.5} /></button>
                                    <button className="rc-btn rc-btn-danger" onClick={() => setDeleteTarget({ ...site, projectCount: count })} title="Delete"><Trash2 size={13} strokeWidth={1.5} /></button>
                                </div>
                            </div>
                        )
                    })}
            ...
            <SiteModal open={modalOpen} onClose={() => setModalOpen(false)} onSave={handleSave} site={editing} />

            {/* Soft-warning delete confirm */}
            {deleteTarget && (
                <div className="modal-overlay" onClick={e => e.target === e.currentTarget && setDeleteTarget(null)}>
                    <div className="modal-box" style={{ maxWidth: 420 }}>
                        <div className="modal-header">
                        ...
                        <div style={{ padding: '12px 0 20px' }}>
                            ...
                            {deleteTarget.projectCount > 0 ? (
                                <div className="st-delete-warning">
                                    ...
                                    <div>
                                        <div style={{ fontWeight: 600, marginBottom: 3 }}>
                                            {deleteTarget.projectCount} project{deleteTarget.projectCount !== 1 ? 's' : ''} reference this site
                                        </div>
                                        <div style={{ fontSize: 12.5 }}>
                                            Those projects will have their site unlinked but won't be deleted. You can reassign them later.
                                        </div>
                                    </div>
                                </div>
                            ) : (
                                <p style={{ fontSize: 13, color: '#9a9790' }}>No projects are using this site. Safe to delete.</p>
                            )}
                        </div>
                        <div className="modal-actions">
                            <button className="btn-ghost" onClick={() => setDeleteTarget(null)}>Cancel</button>
