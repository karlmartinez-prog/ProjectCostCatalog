...
function ResourcePicker({ onSelect }) {
    ...
    function pick(resource) { onSelect(resource); setQuery(''); setResults([]); setOpen(false) }
                        ...
                        <button key={r.id} className="rp-result" onMouseDown={() => pick(r)}>
                            ...
                            <div className="rp-result-info">
                                ...
                                <span className="rp-result-cat">
                                    {r.resource_type}{r.trade && ` · ${r.trade}`}
                                    {r.categories && (
...
export default function ProjectModal({ open, onClose, onSave, project, initialLineItems = [] }) {
    ...
    function addFromCatalog(resource) {
        ...
        const days = isTimeBased ? computeDays(form.start_date, form.end_date, form.working_days_mode) : null
        setItems(prev => [...prev, {
    ...
    function addCustomRow() {
        setItems(prev => [...prev, {
            _id: Math.random().toString(36).slice(2),
            resource_id: null, name: '', unit: '', image_url: '',
            resource_type: 'Material', trade: '',
            unit_cost_snapshot: '', quantity: 1,
            working_days: null, labor_start_date: '', labor_end_date: '',
            category_id: '', category_name: '', category_type: '',
            capex_opex: '', billing_type: 'per_use',
            saveTocatalog: false, isCustom: true,
            _overrideCat: false, _overrideDates: false,
        }])
    }
        ...
        setItems(prev => prev.map(i => {
            ...
            if (field === 'unit') {
                ...
                const days = isTimeBased
                    ? computeDays(i._overrideDates ? i.labor_start_date : form.start_date, i._overrideDates ? i.labor_end_date : form.end_date, form.working_days_mode)
                    : null
            ...
            if (field === 'labor_start_date' || field === 'labor_end_date') {
                ...
                if (start && end) newItem.working_days = computeDays(start, end, form.working_days_mode)
                return newItem
            }
    ...
    function removeItem(id) { setItems(prev => prev.filter(i => i._id !== id)) }

    function itemSubtotal(item) {
        const cost = parseFloat(item.unit_cost_snapshot) || 0
        const qty = parseInt(item.quantity) || 1
        const duration = resolveDurationFromUnit(item.unit, item.working_days || 0)
    ...
    async function handleSubmit(e) {
        ...
        try {
            ...
            for (const item of items.filter(i => i.isCustom && i.saveTocatalog && i.name.trim())) {
                const { data } = await supabase
                    .from('resources')
                    .insert([{ name: item.name, unit_cost: parseFloat(item.unit_cost_snapshot) || 0, unit: item.unit || null, status: 'active', currency: form.currency, resource_type: item.resource_type, category_id: item.category_id || null }])
                    ...
                    <div className="pm-left">
                        ...
                        <div className="pm-form-row">
                            <div className="mf-group">
                                ...
                                <select value={form.site_id} onChange={e => setField('site_id', e.target.value)}>
                            ...
                            <div className="mf-group">
                                <label style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                                    <MapPin size={11} strokeWidth={1.5} style={{ color: '#aaa89f' }} /> Location
                                </label>
                                <input
                                    value={form.location}
                        ...
                        {items.length === 0 ? (
                            ...
                            <div className="pm-items-list">
                                {items.map(item => {
                                    ...
                                    const durationWord = unitNorm === 'per week' || unitNorm === 'week' ? 'week'
                                        : unitNorm === 'per month' || unitNorm === 'month' ? 'month' : 'day'
                                        ...
                                        <div key={item._id} className={`pm-item-card ${isLabor ? 'pm-item-labor' : ''}`}>
                                            <div className="pm-item-top">
                                                <div className="pm-item-thumb">
                                                    {item.image_url ? <img src={item.image_url} alt={item.name} />
                                                        : isLabor ? <HardHat size={13} strokeWidth={1.5} style={{ color: '#c9a84c' }} />
                                                            : <Package size={13} strokeWidth={1.5} />}
                                                </div>
                                                ...
                                                {isLabor && <span className="pm-labor-tag"><HardHat size={10} strokeWidth={2} /> Labor</span>}
                                            ...
                                            <div className="pm-item-fields">
                                                <div className="pm-item-field">
                                                    ...
                                                    <input className="proj-item-input" type="number" min="0" step="0.01"
                                                ...
                                                <div className="pm-item-field" style={{ flex: 2 }}>
                                                    ...
                                                    {!item.isCustom && item.category_name && !item._overrideCat ? (
                                                        ...
                                                        <select className="proj-item-input" value={item.category_id}
                                                ...
                                                <div className="pm-item-field">
                                                    <label>Unit</label>
                                                    {item.isCustom ? (
                                                        <UnitComboField value={item.unit || ''} onChange={(unit, billing_type) => {
                                                            setItems(prev => prev.map(i => i._id === item._id ? { ...i, unit, billing_type } : i))
                                                            updateItem(item._id, 'unit', unit)
                                                        }} placeholder="e.g. per month" />
                                                    ) : (
                                                        <div style={{ padding: '7px 10px', background: '#f9f8f5', borderRadius: 6, fontSize: 13, color: '#7a7872', border: '1px solid #e8e5e0' }}>
                                                            {item.unit || '—'}
                                                        </div>
                                                    )}
                                                </div>
                                                ...
                                                <div className="pm-labor-days-row">
                                                    <div className="pm-labor-days-info">
                                                        ...
                                                        {item._overrideDates ? (
                                                            ...
                                                            <span>
                                                                ...
                                                                <span style={{ color: '#aaa89f', marginLeft: 4 }}>({workingDays} days)</span>
                                                            </span>
                                                        ) : (
                                                    ...
                                                    {!item._overrideDates ? (
                                                        <button type="button" className="pm-labor-override-btn"
                                                            onClick={() => updateItem(item._id, '_overrideDates', true)}>Override dates</button>
                                                    ) : (
                                                        <button type="button" className="pm-labor-override-btn pm-labor-override-clear"
                                                            onClick={() => updateItem(item._id, '_clearDateOverride', true)}>Use project dates</button>
                                                    )}
                                                </div>
                                            )}
                                            ...
                                            {isTimeBased && item._overrideDates && (
                                                <div className="pm-item-fields" style={{ paddingTop: 0 }}>
                                                    <div className="pm-item-field">
                                                        <label>Start</label>
                                                        <input className="proj-item-input" type="date" value={item.labor_start_date}
                                                            onChange={e => updateItem(item._id, 'labor_start_date', e.target.value)} />
                                                    </div>
                                                    <div className="pm-item-field">
                                                        <label>End</label>
                                                        <input className="proj-item-input" type="date" value={item.labor_end_date}
                                                            onChange={e => updateItem(item._id, 'labor_end_date', e.target.value)} />
                                                    </div>
                                                    <div className="pm-item-field" style={{ justifyContent: 'flex-end' }}>
                                                        <label>&nbsp;</label>
                                                        <div style={{ fontSize: 12, color: '#c9a84c', fontWeight: 600, padding: '8px 4px' }}>{workingDays} days</div>
                                                    </div>
                                                </div>
                                            )}

                                            {isTimeBased && parseFloat(item.unit_cost_snapshot) > 0 && duration > 0 && (
                                                <div className="pm-labor-formula" style={!isLabor ? { background: 'rgba(139,92,246,0.06)', borderTopColor: 'rgba(139,92,246,0.15)' } : {}}>
